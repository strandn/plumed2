  ...
  nlist_steps_(0)
{
  ...
  if(getRestart() && (!walkers_mpi_ || multi_sim_comm.Get_rank() == 0)) {
    stopwatch.start("Timing " + std::to_string(getStep() / 50000 + 1));
  }
}
...
void MetaD::update()
{
  if(getStep() % 50000 == 0 && (!walkers_mpi_ || multi_sim_comm.Get_rank() == 0)) {
    if(!isFirstStep_) {
      stopwatch.stop("Timing " + std::to_string(getStep() / 50000));
      log << stopwatch << "\n";
      log.flush();
    }
    stopwatch.start("Timing " + std::to_string(getStep() / 50000 + 1));
